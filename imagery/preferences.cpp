...
QWidget *Preferences::createImageGroup(QWidget *parent) {
	QString imgInfoMsg = tr("<small>Use <b>%c</b> or <b>%C</b> for Image "
							"counter (%3c for padding)</small>");
...
void Preferences::save() {
	...
	QFont font(m_fontNameEdit->text(), m_fontSizeEdit->text().toInt());
	QFont shiftedFont(m_fontShiftedNameEdit->text(), m_fontShiftedSizeEdit->text().toInt());
	setFontDefaults(shifted, font, shiftedFont);

	// color
	QColor fg(m_fontColourFGButton->getColor());
	QColor bg(m_fontColourBGButton->getColor());
	qDebug("fgColor: %s", fg.name(QColor::HexRgb).toLocal8Bit().constData());
	qDebug("bgColor: %s", bg.name(QColor::HexRgb).toLocal8Bit().constData());
	setColorDefaults(fg, bg);
...
void Preferences::getFontDefaults(bool &shifted, QFont &font, QFont &shiftedFont) {
	QFont defFont("CBM", FONT_POINT_BASE);
	QFont defShiftedFont("CBMShift", FONT_POINT_BASE);
