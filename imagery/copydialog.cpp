                          ...
                          WarpCopyDisk::Mode mode,DImage *image)
{
  ...
  QString title;
  switch(mode) {
    case WarpCopyDisk::READ_DISK_SLOW:  title=tr("Reading Disk (Slow)"); break;
    case WarpCopyDisk::WRITE_DISK_SLOW: title=tr("Writing Disk (Slow)"); break;
    case WarpCopyDisk::READ_DISK_WARP:  title=tr("Reading Disk (Warp)"); break;
    case WarpCopyDisk::WRITE_DISK_WARP: title=tr("Writing Disk (Warp)"); break;
  }
  ...
  connect(m_copier,SIGNAL(procBlock(int,int,int,const QString &)),
          this,SLOT(procBlock(int,int,int,const QString &)));
  connect(m_copier,SIGNAL(prepareBlock(int,int)),
          this,SLOT(prepareBlock(int,int)));
  ...
  disconnect(m_copier,SIGNAL(procBlock(int,int,int,const QString &)),
             this,SLOT(procBlock(int,int,int,const QString &)));
  disconnect(m_copier,SIGNAL(prepareBlock(int,int)),
             this,SLOT(prepareBlock(int,int)));
  delete m_copier;
  m_copier = 0;
  ...
  if(ok) {
    if((mode==WarpCopyDisk::READ_DISK_SLOW)||
      ...
      if(numBlockErrors>0) {
        if(QMessageBox::question(this,tr("Read Disk Result"),
      ...
      if(ok)
        image->getFromRawImage(rawImage);
    } else {
      if(numBlockErrors>0) {
...
void CopyDialog::procBlock(int track,int sector,int status,
                           const QString &statusMessage)
{
  // set color block
  QColor color;
  switch(status) {
  case WCB_OK:                   color = Qt::green; break;
  case WCB_NET_ERROR:            color = Qt::darkGray; break;
  case WCB_NET_CHECKSUM_ERROR:   color = Qt::gray; break;
  case WCB_GCR_HEADER_ERROR:     color = Qt::cyan; break;
  case WCB_GCR_DATA_ERROR:       color = Qt::yellow; break;
  case WCB_BLOCK_CHECKSUM_ERROR: color = Qt::magenta; break;
  case WCB_HARD_ERROR:           color = Qt::red; break;
  ...
  // set status
  QString msg;
  msg.sprintf("Track %02d, Sector %02d ",track+1,sector);
  msg += statusMessage;
...
void CopyDialog::updateTime()
{
  ...
  int total = m_startTime.secsTo(now);
  int secs = total % 60;
  total /= 60;
  int mins = total % 60;
  total /= 60;
  QString time;
  time.sprintf("%02d:%02d:%02d",total,mins,secs);
