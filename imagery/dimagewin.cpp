...
: MainWin("DImageWin", QRect(300, 100, DEFAULT_WIDTH, DEFAULT_HEIGHT), parent),
	m_dimage(format) {
...
void DImageWin::init() {
	...
	m_dirView->header()->setMinimumSectionSize(font.pointSize()*12);
	m_dirView->header()->setDefaultSectionSize(font.pointSize()*12);
...
void DImageWin::updateFont() {
	...
	// set actions
	if (m_fontShifted != m_shiftCharsetAction->isChecked()) m_shiftCharsetAction->setChecked(m_fontShifted);
}

void DImageWin::updateDImage() {
	bool isDirty = m_dimage.isDirty();
	if (m_dimage.isFileAvailable()) setWindowIcon(isDirty ? m_darkFileIcon : m_fileIcon);
	else setWindowIcon(QIcon(":/imagery/imagery-16.png"));
...
bool DImageWin::saveImageAs() {
	...
	filter << tr("All Files (*.*)");
	QString fileName = QFileDialog::getSaveFileName(this, tr("Save Image as"), m_dimage.fileName(), filter.join(";;"));
	if (fileName != "") {
...
bool DImageWin::saveFileAs() {
	...
	filter << tr("All Files (*.*)");
	QString fileName = QFileDialog::getSaveFileName(this, tr("Save File as"), file.convertToAsciiName(), filter.join(";;"));
...
void DImageWin::closeEvent(QCloseEvent *event) {
	if (m_dimage.isDirty()) {
		int result = QMessageBox::warning(
			...
			tr("Disk Image '%1' is not saved!").arg(QFileInfo(m_dimage.fileName()).fileName()),
...
void DImageWin::showCharset() {
	...
	m_charsetDialog->setWindowTitle(
		(m_fontShifted)? tr("Shifted CBM Charset") : tr("Unshifted CBM Charset"));
...
void DImageWin::formatDisk() {
	...
	if (m_formatImageDialog->exec() == QDialog::Accepted) {
		...
		int idSize = id.size();
		if ((idSize != 0) && (idSize != 2)) {	// invalid id
			QMessageBox::warning(this, tr("Format Image"), tr("Invalid disk id given!"));
		} else {
			m_dimage.format(name, id);			// format image
			updateDImage();
		}
	}
}
...
void DImageWin::addSeparator() {
	...
	if (m_addSeparatorDialog->exec() == QDialog::Accepted) {
		...
		if (sep == "") QMessageBox::warning(this, tr("Add Separator"), tr("Ignoring empty separator!"));
		else {
			CBMFile sepFile(sep, CBMFile::DEL);			// add separator
...
void DImageWin::mountImage() {
	if (!saveImage()) {
		QMessageBox::warning(this, tr("Mount Image"), tr("You need to save an Image before mounting it!"));
		return;
	}
...
void DImageWin::runProgram() {
	...
	if (selection.size() != 1) {
		QMessageBox::warning(this, tr("Run Program"), tr("You need to select a single Program before running it!"));
		return;
	}
	if (!saveImage()) {
		QMessageBox::warning(this, tr("Run Program"), tr("You need to save an Image before mounting it!"));
		return;
	}
...
void DImageWin::runEmu(const QString &app, const QString &args, const QString &fileName) {
	QStringList cmdLine;							// create command line
	...
	for (int i = 0; i < size; i++) {
		...
		if (str[i] == '%') {
			if (i < (size - 1)) {
				...
				if (nextChar == 'i') {				// image path name
					result += m_dimage.fileName();
					i++;
					replaced = true;
				} else if (nextChar == 'P') {		// program name in petscii
					result += fileName;
					i++;
					replaced = true;
				} else if (nextChar == 'p') {		// program name in unicode/ascii
...
bool DImageWin::getCurrentFiles(CBMFileList &files) {
	QModelIndexList selection = m_dirView->selectionModel()->selectedRows();
	if (selection.empty()) m_dimage.readDirectory(files);	// read full directory of image
	else {
		QString name, id;
