...
void DImageWin::init()
{
  ...
  connect(m_dirView,SIGNAL(activated(const QModelIndex &)),
...
{
  if(m_dimage.isDirty()) {
    int result = QMessageBox::warning(this,tr("Unsaved Image"),
...
void DImageWin::showCharset()
{
  ...
  m_charsetDialog->setWindowTitle(m_fontShifted ? tr("Shifted CBM Charset") :
