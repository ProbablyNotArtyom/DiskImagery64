...
{
  connect(service,SIGNAL(networkEvent(const QString &)),
          this,SLOT(logMessage(const QString &)));
  connect(service,SIGNAL(sentPacket(int)),
          this,SLOT(reportOutPacket(int)));
  connect(service,SIGNAL(receivedPacket(int)),
          this,SLOT(reportInPacket(int)));
}
