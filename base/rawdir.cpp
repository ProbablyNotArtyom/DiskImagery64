...
bool RawDir::toFileList(CBMFileList &fileList) {
    ...
    // check header
    QString name, id;
    if (!readHeader(name, id)) return false;
    name = name.trimmed();
    id = id.trimmed();
    ...
    forever {
        ...
        if (w == 0) break;
        if (w != 0x0101) return false;

        quint16 blocks;
        if (!readWord(blocks)) return false;
        quint8 b;
        ...
        CBMFile file;
        if (!readFile(file, blocks)) return false;
        fileList << file;
    }
    return true;
}
...
bool RawDir::readHeader(QString &name, QString &id) {
    ...
    if (!checkWord(0x0000)) return false;
    if (!checkByte(0x12)) return false;
    if (!checkByte('"')) return false;
    if (!readString(16, name)) return false;
    if (!checkByte('"')) return false;
    if (!readByte(b)) return false;
    if (!readString(2, id)) return false;
    for (int i = 0; i < 3; i++)
        if (!readByte(b)) return false;
...
bool RawDir::readFile(CBMFile &file, quint16 blocks) {
    QByteArray line;
    if (!readBytes(27, line)) return false;
...
void RawDir::fromFileList(const CBMFileList &fileList) {
    ...
    if (!m_skipLoadAddr) writeWord(0x0401);
    writeWord(0x0101);
    writeWord(0x0000);
    writeByte(0x12);
    writeByte('"');
    ...
    for (int i = 0; i < fileList.size(); i++) {
        ...
        blocks = file.blocks();
        blockDigits = int(calcNumDigits(quint16(blocks)));
        int spaces = 4 - blockDigits;
        QString name = file.name();
        QString type = file.convertTypeToString();

        writeWord(0x0101);
        writeWord(quint16(blocks));
    ...
    // blocks free
    writeWord(0x0101);
    writeWord(quint16(fileList.freeBlocks()));
    ...
    // end
    writeWord(0);
    m_dirBuffer.resize(int(m_dirBufPos));
}

// ----- Tools -----

bool RawDir::readByte(quint8 &b) {
    quint32 dirBufSize = quint32(m_dirBuffer.size());
    if (m_dirBufPos >= dirBufSize) return false;
    b = quint8(m_dirBuffer[m_dirBufPos++]);
    return true;
}
...
bool RawDir::readString(int size, QString &str) {
    quint32 dirBufSize = quint32(m_dirBuffer.size());
    if ((m_dirBufPos + static_cast<unsigned int>(size)) >= dirBufSize) return false;
    QByteArray ba;
    ba.resize(size);
    for (unsigned int i = 0; i < static_cast<unsigned int>(size); i++)
        ba[i] = m_dirBuffer[m_dirBufPos + i];

    m_dirBufPos += static_cast<unsigned int>(size);
...
bool RawDir::checkWord(quint16 w) {
    quint16 x;
    if (!readWord(x)) return false;
    return (x == w);
}

void RawDir::writeByte(quint8 b) {
    quint32 dirBufSize = quint32(m_dirBuffer.size());
    if (m_dirBufPos >= dirBufSize) m_dirBuffer.resize(int(dirBufSize) + 1024);
    m_dirBuffer[m_dirBufPos++] = char(b);
}
...
void RawDir::writeString(const QString &string, int pad) {
    ...
    if (size > pad) size = pad;
    for (int i = 0; i < size; i++)
        writeByte(quint8(string[i].unicode()));
}
...
int RawDir::calcNumDigits(quint16 blocks) {
    int digits;
    if (blocks > 99) digits = 3;
    else if (blocks > 9) digits = 2;
    else digits = 1;
    return digits;
}
