...
DImage::DImage(const QString &fileName) : m_fileName(fileName) {
    ...
    if (m_format != INVALID) {
        ...
        m_dirty = false;
        if (m_diskImage == nullptr) createNewImage();
...
QString DImage::status() const {
    QString result;
    if (m_diskImage == nullptr) return result;
...
void DImage::format(const QString &name, const QString &id) {
    ...
    // quick format
    else di_format(m_diskImage, rawName, nullptr);

    m_dirty = true;
}

bool DImage::readDirectory(CBMFileList &files) {
    if (m_diskImage == nullptr) return false;

    files.clear();

    unsigned char rawName[17] = "$";
    char mode[3] = "rb";
    ImageFile *fh = di_open(m_diskImage, rawName, T_PRG,  mode);
    ...
    while (di_read(fh, buffer, 254) == 254) {
        for (int pos = 0; pos < 254; pos += 32) {
            if (buffer[pos] != 0) {
                ...
                // blocks size
                int blocks = (int(buffer[pos + 29])) << 8 | int(buffer[pos + 28]);
...
bool DImage::readFile(CBMFile &file) {
    ...
    ImageFile *fh = di_open(m_diskImage, rawName, ft, mode);
    if (fh == nullptr) return false;
    ...
    if (file.type() != CBMFile::DEL) {
        ...
        while ((len = di_read(fh, buffer, 4096)) > 0) {
            ...
            memcpy(data.data() + total, buffer, size_t(len));
            total += len;
        }
        file.setData(data);
    }
...
bool DImage::writeFile(const CBMFile &file) {
    ...
    if (file.type() != CBMFile::DEL) {
        ...
        if (data.size() > 0) {
            if (di_write(fh, (unsigned char *)data.data(), data.size()) != data.size())
                return false;
        }
    }
    di_close(fh);
...
int DImage::blocksFree() const {
    if (m_diskImage == nullptr) return 0;
...
DImage::DiskFormat DImage::determineDiskFormat(const QString &fileName) {
    ...
    if (ext.size() != 4) return INVALID;
    if (ext[0] != '.') return INVALID;
    if ((ext[1] != 'd') && (ext[1] != 'D')) return INVALID;
...
void DImage::convertFileNameToRaw(const QString &str, unsigned char *raw) const {
    QByteArray ba;
    int size = str.size();
    if (size > 16) size = 16;
    ba.resize(size + 1);
    for (int i = 0; i < size; i++)
        ba[i] = char(str[i].unicode());
...
OffsetMap DImage::offsetMap(int *numBlocks) const {
    ...
    if (numBlocks != nullptr) *numBlocks = sum;
    return om;
}

int DImage::blockNum(int track, int sector) const {
    TrackSector ts = {static_cast<unsigned char>(track + 1), static_cast<unsigned char>(sector)};
...
void DImage::putToRawImage(QByteArray &image) {
    ...
    memcpy(image.data(), m_diskImage->image, size_t(m_diskImage->size));
}

bool DImage::getFromRawImage(const QByteArray &image) {
    if (image.size() != m_diskImage->size) return false;
    memcpy(m_diskImage->image, image.constData(), size_t(m_diskImage->size));
    m_dirty = true;
    return true;
}
