...
void WarpCopyTools::encodeBlock(const QByteArray &realBlock,
                                QByteArray &rawBlock, int track, int sector) {
    ...
    // (throw away first gcr byte of encoded 0x07 (CBM Data Block) = always
    // 0x55) (throw away last 2 gcr bytes of encoded 0x00 0x00 = always 0x29
    // 0x4a)
    // -> 322 gcr bytes left
    for (int i = 0; i < 322; i++) {
...
int WarpCopyTools::decodeBlock(const QByteArray &rawBlock,
                               QByteArray &realBlock, int &track, int &sector,
                               bool decodeSector) {
...
void WarpCopyTools::unpackBlock(const QByteArray &rawBlock,
                                QByteArray &gcrBlock, char &gcrSector,
                                bool &lastSector) {
    const char nybbleTrafo[16] = {0xf, 0x7, 0xd, 0x5, 0xb, 0x3, 0x9, 0x1,
