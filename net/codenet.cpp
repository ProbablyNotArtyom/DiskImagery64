...
CodeNetTask::CodeNetTask(const QByteArray &packet)
: m_packet(packet)
{
}
...
{  
  ...
  if(host.sendPacket(m_packet)) {
    ...
    while(host.receivePacket(m_packet)) {
      ...
      if((m_packet[0]==(char)0xca)&&
         (m_packet[1]==(char)0x1f)&&
         (m_packet[2]==(char)seqNum)&&
         (m_packet[3]<(char)2)) {
        // packet ok
        bool ok = (m_packet[3]==(char)1);
        if(!ok) {
          setStatusMessage(QObject::tr("packet returned error status!"));
          return ERROR;
        }
        return READY;
      }
...
void CodeNetService::sendData(quint16 addr,const QByteArray &data)
{
  ...
  reportEvent(tr("sending data (%1 bytes)").arg(QString::number(data.size())));
}
...
void CodeNetService::execJump(quint16 addr)
{
  ...
  m_packet[5] = lo(addr);
  sendCommand(6);
  reportEvent(tr("execute jump to 0x%1")
              .arg(QString::number(addr,16)));
}
