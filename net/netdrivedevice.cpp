...
NetDriveDevice::NetDriveDevice(const CBMFileList &files) : m_files(files) {
    initStreams();
    setMessage(OK);
    m_dirBuffer.resize(4096);
}

NetDriveDevice::~NetDriveDevice() { freeStreams(); }

// interface for net drive

NetDriveDevice::Result NetDriveDevice::open(const QString &name,
                                            quint8 channel) {
    OpenMode openMode;
        ...
        if ((fileName == "$") && (openMode == READ)) {
            ...
            if (openMode == READ) {
                ...
                if (pos == -1) {
                    CBMFile file(fileName, fileType);
                    m_files << file;
                    openStream(channel, m_files.size() - 1, file.data(),
                               APPEND);
                } else {
...
QString NetDriveDevice::parseTypeMode(const QString &name, CBMFile::Type &type,
                                      OpenMode &mode) {
    QString result;
...
void NetDriveDevice::openStream(quint8 channel, int fileId,
                                const QByteArray &data, OpenMode openMode) {
...
void NetDriveDevice::setMessage(Message msg) {
    ...
    case FILE_TYPE_MISMATCH:
        txt = "FILE TYPE MISMATCH";
        break;
    case NO_CHANNEL_AVAILABLE:
        txt = "NO CHANNEL AVAILABLE";
        break;
    }
...
NetDriveStream::NetDriveStream(int fileId, const QByteArray &data,
                               NetDriveDevice::OpenMode openMode)
    : m_fileId(fileId), m_data(data), m_openMode(openMode), m_pos(0),
      m_lastPos(0) {}

NetDriveStream::~NetDriveStream() {}

void NetDriveStream::offset(quint8 pos) { m_pos = m_lastPos + pos; }
